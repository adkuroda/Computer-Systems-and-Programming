...
int insert_ascend(Schedule *schedule, Process *process);
Queue * queue_init();
...
int scheduler_add(Schedule *schedule, Process *process) {
  ...
  for (i = 1; i <= 3; i++){
    if ((process -> flags) & (mask << i)){
      ...
      return insert_ascend(schedule, process);
    }
  }
  return -1;
}
...
Process *scheduler_select(Schedule *schedule) {
  ...
  if ((clock_get_time() - current -> time_last_run) >= TIME_STARVATION){
    ...
    current -> next = NULL;
    current -> flags &= ~READY;
    current -> flags |= RUNNING;
    return current;
  }
  //check for time starvation first
  while (current -> next != NULL){
    if  ((current -> next -> time_last_run - clock_get_time()) >= TIME_STARVATION){
  ...
  int pid_smallest = current -> pid;
  int s_time_remaining = current -> time_remaining;
  current = current -> next;
  while(current != NULL){
    if (current ->time_remaining < s_time_remaining){
      pid_smallest = current -> pid;
      s_time_remaining = current -> time_remaining;
    }
    if (current ->time_remaining == s_time_remaining){
      if(current -> pid < pid_smallest){
        pid_smallest = current ->pid;
      }
    }
    current = current -> next;
  ...
  while(current -> next != NULL){
    if(current -> next ->pid == pid_smallest){
      schedule ->ready_queue -> count -= 1;
...
int scheduler_io_run(Schedule *schedule) {
  ...
  schedule ->waiting_queue -> head = schedule ->waiting_queue -> head -> next;
