...
int exit_code = 0;
int job_id = 1;
int num_jobs = 0; /*Keeps track of number of jobs*/
...
int file_redirection(Cmd_aux aux,char *cmd, char *argv[]);
void free_list();
void fg(char *cmd, char *argv[]);
    ...
    if(!is_whitespace(cmd)) {
      ...
      if (strcmp(argv[0],"help") == 0){
        ...
        if (aux.is_bg){
          ...
          insert_node(cmd, job_id, pid_bg, status_running);
          ...
          sigprocmask(SIG_BLOCK, &mask_all, &prev_all);
          fg_job = create_node(cmd, 0, pid_fg, status_running);
          sigprocmask(SIG_SETMASK, &prev_all, NULL);
...
/* This function processes the fg built in function. if there is an no such job ID 
 */
void fg(char *cmd, char *argv[]){
  int temp_JOBID = atoi(argv[1]);
  int PID = -1;
  Node * current = head;
  while (current != NULL){
    if (temp_JOBID == current -> job_ID){
  ...
  if (PID == -1){
    ...
    if (strcmp(current -> state, status_stopped) == 0){
        kill(PID, SIGCONT);
        fg_job = get_node(PID);
    }
    ...
    if (WIFEXITED(status)){
        log_job_state(PID, LOG_FG, cmd, LOG_TERM);
        remove_node(PID);
    }
    else if (WIFSIGNALED(status)){
        log_job_state(PID, LOG_FG, cmd, LOG_TERM_SIG);
        remove_node(PID);
    }
  }
}
...
int file_redirection(Cmd_aux aux, char *cmd, char *argv[]){
  if (aux.in_file != NULL){
    int fd = open(aux.in_file, O_RDONLY, 0600);
    if (fd < 0){
  ...
  if (aux.out_file != NULL){
    int fd1 = -1;
    if (aux.is_append > 0){
      fd1 = open(aux.out_file, O_RDWR, 0600);
      if (fd1 < 0){
    ...
    fd1 = open(aux.out_file, O_WRONLY, 0600);
    if (fd1 < 0){
      log_file_open_error(aux.out_file);
      exit(0);
    }
...
/*Frees all the allocated memory when it quits the MASH. Ensures there is no
 * mememory leak 
 */
void free_list() {
  Node * current = head;
